...
# its group is "staff".  Since the user's primary group ID isn't the same as
# the user ID, for sshd to accept ".ssh/authorized_keys", it must not be
# group-writable.
#
...
# group-writable by the private group.  (sshd is fine with that arrangement
# too, since it's still not writable by _others_.)
#
...
# permissions.  Once written out on the Helios side, sshd will reject the
# authorized_keys file as having permissions that are too loose, and the user
# will then fail to log in via ssh.
#
